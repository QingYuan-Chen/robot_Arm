...
def test_active_source_contains_upstream_core_and_current_compatibility_entrypoints():
...
def test_moveit_sim_wrapper_selects_upstream_by_default_and_keeps_current_fallback():
    launch_text = (ACTIVE / "launch/mujoco_moveit_sim.launch.py").read_text(encoding="utf-8")

    assert 'DeclareLaunchArgument("simulation_backend", default_value="upstream")' in launch_text
