...
def test_single_bottle_grasp_bottle_plan_accepts_point_four_confidence_boundary() -> None:
...
spec = importlib.util.spec_from_file_location('single_bottle_confidence_test', {str(RUNNER_PATH)!r})
...
def test_single_bottle_grasp_bottle_plan_accepts_85mm_jaw_boundary() -> None:
    script = f"""
import importlib.util
from rebotarm_msgs.msg import GraspPlan

spec = importlib.util.spec_from_file_location('single_bottle_jaw_width_test', {str(RUNNER_PATH)!r})
assert spec is not None and spec.loader is not None
runner = importlib.util.module_from_spec(spec)
spec.loader.exec_module(runner)
...
def test_single_bottle_grasp_runner_default_open_effort_matches_web_teleop() -> None:
    script = f"""
import importlib.util
import sys

spec = importlib.util.spec_from_file_location('single_bottle_open_effort_test', {str(RUNNER_PATH)!r})
assert spec is not None and spec.loader is not None
runner = importlib.util.module_from_spec(spec)
spec.loader.exec_module(runner)
sys.argv = [
    ...
    '--gripper-open-m', '0.08',
    '--output', '/tmp/single-bottle-parser-test.json',
]
