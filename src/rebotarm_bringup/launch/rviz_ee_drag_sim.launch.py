"""纯仿真下的 RViz 末端拖动入口（不接真实硬件）。

用途：在 RViz 里用 MoveIt 原生的交互标记拖动机械臂末端，验证规划与执行链路，
全程不打开任何硬件通道，也不依赖真实电机反馈。

启动组合：
    1. 本包内的交互系统共享启动文件（真机 / 仿真二选一的后端选择逻辑都在其中），
       这里显式选择「仿真预览」一侧；
    2. 仿真轨迹控制器节点，顶替真机执行层担任 ``follow_joint_trajectory`` 动作服务端，
       并按固定频率发布关节状态，让 RViz 里的模型真正动起来。

后端选择逻辑：``use_hardware=false`` 关闭真机通道，``use_moveit_preview=true`` 让共享
启动文件包含规划包自带的预览栈（提供规划能力与假关节状态），因此本入口只能用于可视化与
上层流程联调，不构成对真实控制器的验证。

安全默认值：``start_passive_joint_state_publisher=false`` 且 ``use_moveit_fake_joint_states=false``，
即不再引入额外的假关节状态源——仿真轨迹控制器是本入口唯一的关节状态发布者，同时也是规划
执行所依赖的动作服务端；两个状态源同时存在会让规划器看到互相矛盾的关节状态。
"""
