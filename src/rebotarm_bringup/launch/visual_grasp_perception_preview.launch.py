...
用途
----
把“相机 → YOLO 检测 → GraspNet 候选 → 逆解过滤 → 可视化标记”这条感知与候选链路单独拉
起来，配合本地 RViz 看抓取候选与夹爪位姿。用于装机与标定后的只读验收：确认相机、深度、
检测、候选与坐标系朝向是否合理，不代表抓取一定可执行。

启动内容（节点组合）
--------------------
1. 主视觉链路：以子启动方式引入视觉包的视觉启动文件（``start_vision``，默认 true），
   参数透传相机配置、手眼配置、YOLO 权重与普通抓取开关；
2. 抓取候选推理节点：以 ``graspnet_python_executable`` 指定的独立解释器前缀运行，模型路径
   由环境变量或工作区相对路径给出（``start_graspnet_baseline``，默认 true）；
3. 候选逆解过滤节点：先订阅候选话题做逆解与（可选）碰撞检查，再发布过滤后的候选与抓取计划
   （``start_candidate_ik_filter``，默认 true）；该节点只调用逆解与状态有效性服务，不产生运动；
4. 可视化标记节点：把抓取计划画成标记数组，供 RViz 查看（``start_visual_grasp_markers``，默认 true）；
5. 本地 RViz（``use_local_rviz``，默认 true），加载本包的抓取预览配置文件。

真实/仿真后端选择逻辑
----------------------
本文件刻意不启动任何执行后端：既不启动真实硬件控制器，也不启动仿真的轨迹控制器、规划节点
或任何执行入口。唯一的“后端相关性”来自两个坐标/话题约定：候选过滤节点的目标坐标系固定为
``base_link``，关节状态来自 ``candidate_joint_state_topic`` 指向的话题（默认
``/rebotarm/visual_joint_states``，由外部真机或仿真侧提供）。因此本预览在真机与仿真下都能用，
但都必须保证该话题有有效关节反馈，否则逆解会因缺少种子状态而失败。

参数来源与优先级
----------------
- 参数文件：抓取姿态策略、台面安全两份配置加载到逆解过滤节点，GraspNet 配置单独加载到推理节点；
  参数文件里的键若在下面的节点参数字典中再次出现，以本文件的取值为准；
- 本文件对参数的显式设定中，几处安全相关的关键值分别是
  ``candidate_workspace_gate_enabled=true``（工作空间盒闸门，比参数文件里的 false 更严格）、
  ``candidate_min_confidence=0.4``（过滤低置信度候选）、
  ``candidate_max_joint6_delta_rad=0.0``（不允许候选改变末轴角度）、
  ``collision_check_enabled=false``（对齐 ``candidate_collision_check_enabled``；本预览不依赖
  规划场景服务，只做逆解可行性判断）；
...
说明：本文件不启动任何执行后端，也没有实现电机控制、运动规划或感知算法，这些分别属于控制器、
运动、视觉包与标定包的职责。
"""
import os
