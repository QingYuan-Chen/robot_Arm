...
#
# 用途：一条命令拉起 就绪摆位 → 相机与检测 → 抓取网络候选 → MoveIt 逆解与碰撞过滤
#      → 点到点执行 → 抓取执行器 的完整链路，并在 RViz 中显示数据与标记。
#
# 节点组合与启动顺序：
#   1) 先包含共享底层栈（MoveIt 规划、RViz、真实或仿真的关节状态来源）；
#   2) 再启动一次性的视觉就绪实例，把机械臂摆到观察位姿后退出；
#   3) 用进程退出事件在它退出后启动后续全部视觉抓取节点；若 start_visual_ready=false，
#      则用取反条件直接启动这些节点，跳过摆位。
#   这样可保证相机与检测只在手眼标定所用的位姿下才开始工作。
#
# 真实/仿真后端选择：
#   - use_hardware=true：共享底层栈启动真实电机控制器，轨迹执行落在硬件层；
#     同时本文件的仿真轨迹控制器被条件排除，避免出现两个同名轨迹服务。
#   - use_hardware=false（默认）：由共享底层栈提供 MoveIt 或仿真的关节状态；
#     仿真轨迹控制器只在 start_sim_trajectory_controller=true 且非真实硬件时启动。
#
# 参数来源：
