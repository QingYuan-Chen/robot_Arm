...
source /home/a/project/rebot_Arm-worktrees/gripper-unified-bus-fix/install_decoupling/setup.bash
...
本次验证安装在工作树的 `install_decoupling`，因为原有 `install` 是指向主目录
旧构建的链接。上述新 overlay 使用主目录作为依赖 underlay，只覆盖本次修改的
bringup、simulation、vision 三包；原 `install` 链接保持不变。重建命令为：

```bash
cd /home/a/project/rebot_Arm-worktrees/gripper-unified-bus-fix
source /opt/ros/jazzy/setup.bash
source /home/a/project/rebot_Arm/install/setup.bash
colcon --log-base log_decoupling build --base-paths src \
  --build-base build_decoupling --install-base install_decoupling --symlink-install \
  --packages-select rebotarm_bringup rebotarm_simulation rebotarm_vision \
  --allow-overriding rebotarm_bringup
```
